    ...
    return {
        ...
        "properties": {
            ...
            "backend_options": {
                ...
                "description": 'External CLI launch options only; ignored by lingtai backends and not applied to `ask`. Scalars/lists map to flags (true is a flag alone, e.g. {"search": true} → --search; string/int/float → \'--flag <value>\'; list of scalars repeats the flag; false/null omits it); underscores in keys become dashes. Only the reserved `env` key accepts an object (string name -> string value, injected into the spawned CLI subprocess, e.g. profile selection); other nested objects and unsafe keys are rejected. Discover supported flags with each CLI\'s own --help (e.g. \'claude --help\', \'codex exec --help\') since the flag list changes between versions — this field is intentionally a passthrough rather than a fixed list. See daemon-manual.',
            },
            "task_files": {
                "type": "array",
                "items": {
