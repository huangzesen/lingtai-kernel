...
| `cli_daemon.py` | `lingtai-agent daemon emanate|list|check|ask|wait|reclaim` — the external-owner (shell/Python/CI/coding-agent) skin over the daemon engine. Every command takes `--owner-dir` (legacy spelling `--agent-dir`, one argparse destination): any directory with its own `init.json`, running Agent not required, no `.agent.lock` lease taken; run state and supervisor notifications stay under that owner directory. `_CliDaemonAgent` is the minimal parent-agent facade `DaemonManager` reads (no lease, heartbeat, or agent identity), built from the owner's effective config through the canonical `init_reader.read_init`; `emanate` validates the tasks file against the tool's own emanate schema and enforces the preset allowlist and effective capability policy before previewing, then dispatches through the `DaemonFamilyDispatcher` envelope only under `--yes`; `ask` and `reclaim` route through the same envelope; `_ReadOnlyDaemonView` binds the manager's unmodified `_handle_list`/`_handle_check` with both of its write paths (startup reconciliation, lazy daemon.json repair) removed, and `wait` uses the view once to resolve the run, polls only atomic `daemon.json`, then uses one terminal full check (each progress change once, JSONL under `--json`, exit 0/1/124/130) without constructing a manager |
...
**`cli_daemon.py`**: `_CliDaemonAgent` (`for_dispatch` reads effective config via `init_reader.read_init`; `for_inspection` reads none) · `_CliDaemonAgent.effective_capabilities` / `install_tool_surface` (`registry.apply_core_defaults` — honors `manifest.disable` and authored kwargs) · `_ReadOnlyDaemonView` (overrides `_load_or_rebuild_daemon_state` so inspection reconstructs in memory instead of repairing on disk) · `_resolve_owner_dir` · `_read_effective_init` · `_load_tasks_file` · `_check_schema` / `_validate_emanate_input` (interprets the tool's own `_emanate_input_schema` at preview time) · `_enforce_preset_allowlist` · `_enforce_capability_policy` · `_dispatch_through_tool_family` · `_handle_ask` · `_handle_wait` / `_progress_signature` / `_wait_record` (read-only poll, change-once reporting, exit-status mapping) · `_add_owner_dir_argument` · `add_daemon_parser` · `handle_daemon_command`
