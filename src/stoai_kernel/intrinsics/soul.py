...
def whisper(agent) -> str | None:
    ...
    try:
        ...
        response = session.send(
            f"{agent._soul_prompt}\n\n[Text output budget: 1000 tokens. Be brief, you are the second self of the agent, answer the inquiry as inner voice, you don't have tools, do not attempt tool calls.]"
        )
    except Exception:
        return None
